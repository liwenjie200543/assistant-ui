...
- [#7845](https://github.com/assistant-ui/assistant-ui/pull/7845) [`a31c760`](https://github.com/assistant-ui/assistant-ui/commit/a31c7601deccca97d56d07728daa9adddc32ff59) - fix: `useAdkSubmitAuth` answers the pending `adk_request_credential` call with its auth config carrying the credential as `exchangedAuthCredential`, the reply ADK resumes the tool on; an id that is not a pending request throws ([@okisdev](https://github.com/okisdev))

- [#7900](https://github.com/assistant-ui/assistant-ui/pull/7900) [`ad0b87f`](https://github.com/assistant-ui/assistant-ui/commit/ad0b87f7f4ec95a3f958bcf5f7c7249f55b3c473) - fix: skip malformed ADK message content instead of throwing ([@Kinfe123](https://github.com/Kinfe123))

- [#7376](https://github.com/assistant-ui/assistant-ui/pull/7376) [`a4a6c50`](https://github.com/assistant-ui/assistant-ui/commit/a4a6c509cf61e20f6cd9060838d9ad150023ad7c) - fix: respect response backpressure when streaming ADK events to slow readers. ([@Kinfe123](https://github.com/Kinfe123))
...
- [#7798](https://github.com/assistant-ui/assistant-ui/pull/7798) [`56479fc`](https://github.com/assistant-ui/assistant-ui/commit/56479fcad50924cc4b2cfd8b5a5dd0dc40b87c00) - fix: allow cancellation while dynamic request headers are resolving ([@Kinfe123](https://github.com/Kinfe123))
...
- [#7505](https://github.com/assistant-ui/assistant-ui/pull/7505) [`570a0d3`](https://github.com/assistant-ui/assistant-ui/commit/570a0d3229acffefab8c8a0ea9faecccdf5aac67) - fix: give a function call that arrives without `args` an empty args object, so an args-less call no longer renders as a tool call with undefined `args` and `argsText` ([@Kinfe123](https://github.com/Kinfe123))
...
- [#7668](https://github.com/assistant-ui/assistant-ui/pull/7668) [`23308d1`](https://github.com/assistant-ui/assistant-ui/commit/23308d14df97e262033efa8346cb193898d9f13f) - fix: list only unanswered requests in `useAdkToolConfirmations`, `useAdkAuthRequests` and session snapshots, derived from the `adk_request_confirmation` and `adk_request_credential` calls in the thread, so a session load no longer lists settled requests, a later send no longer drops unanswered ones, and a confirmation no longer appears twice. an auth request's `toolCallId` is now the id of its `adk_request_credential` call, the only id ADK accepts a credential reply on (it was the gated tool call's id with ADK JS), and `authConfig` is also read from camelCase args. ([@okisdev](https://github.com/okisdev))
